...
class PaginatorCallback[T](CallbackData, prefix="paginator"):
    ...
    def add_base_pagination_buttons(
        ...
        has5next_cd = (
            self.make(next5offset) if has5next else self.switch_to_last(length)
        )
        ...
        builder.row(
            # В самое начало
            IKButton(
                text=PAGE_BIG_BACK_SYMBOL, callback_data=self.switch_to_first().pack()
            ),
            # Назад на 5 страниц
